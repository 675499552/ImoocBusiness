...
public class ImageLoaderManager {
    ...
    private ImageLoaderManager(Context context){
        ImageLoaderConfiguration configuration = new ImageLoaderConfiguration.
                ...
                .threadPriority(Thread.NORM_PRIORITY - PROPRITY)//设置图片下载优先级比系统默认优先级低一级
    ...
    private DisplayImageOptions getDefaultOptions(){
        DisplayImageOptions options = new DisplayImageOptions.Builder()
                .showImageForEmptyUri(R.drawable.xadsdk_img_error)//在图片地址为空的时候加载的图片
                .showImageOnFail(R.drawable.xadsdk_img_error)//在图片下载失败的时候加载的图片
                .cacheInMemory(true)//设置图片可以缓存在内存
                .cacheOnDisk(true)//设置图片可以缓存在硬盘
                .bitmapConfig(Bitmap.Config.RGB_565)//使用的图片的解码类型
